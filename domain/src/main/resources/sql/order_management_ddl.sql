...
CREATE TABLE coupon (
    ...
    code VARCHAR(50) DEFAULT NULL COMMENT '优惠券码',
...
CREATE TABLE user_coupon (
    ...
    status VARCHAR(20) DEFAULT '未使用' COMMENT '使用状态(未使用/已使用/已过期)',
    order_id BIGINT DEFAULT NULL COMMENT '使用订单ID',
    order_no VARCHAR(50) DEFAULT NULL COMMENT '使用订单编号',
    use_time DATETIME DEFAULT NULL COMMENT '使用时间',
    get_type VARCHAR(20) DEFAULT '主动领取' COMMENT '获取方式(主动领取/系统发放/活动赠送)',
    version INT DEFAULT 1 COMMENT '版本号',
    created_at DATETIME DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    updated_at DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '修改时间'
) COMMENT='用户优惠券表'; 